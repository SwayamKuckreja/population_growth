...
@purpose: model stochastic population growth using milstein scheme
...
def milstein(c_factor, dt=0.001, T=1.0, num_trials=1000):  # Set simulation time, fine dt and number of trials before averaging
    ...
    for q in range(num_trials):  # Run the same dtc alot of times to average
        ...
        for i in range(n - 1):
            ...
            y[i + 1] = y[i] + (b[i] * y[i] - d[i] * y[i]) * dt + sqrt_term * (w1[i+1] - w1[i]) + 0.5 * (1 / (2 * sqrt_term)) * (((w1[i+1] - w1[i])  ** 2) - dt)
        ...
        for i in range(nc - 1):
            ...
            yc[i + 1] = yc[i] + (bc[i] * yc[i] - dc[i] * yc[i]) * dtc + sqrt_term_c * (w1c[i + 1] - w1c[i]) + 0.5 * (1 / (2 * sqrt_term_c)) * (((w1c[i + 1] - w1c[i]) ** 2) - dtc)
